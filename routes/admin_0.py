...
def admin_dashboard():
    ...
    try:
        ...
        # Count pending verifications (e.g., users with status 'pending')
        cur.execute("SELECT COUNT(*) FROM users WHERE status = %s", ("pending",))
        ...
        total_users = total_businesses = pending_verifications = 0
    finally:
        conn.close()
...
def admin_users():
    ...
    conn = get_db_connection()
    users, user_requests, business_requests = [], [], []
    pending_user_registration_request_count = 0
    pending_business_registration_requests_count = 0
    ...
    if conn:
        try:
            ...
            cur.execute("SELECT profile_pic FROM admin WHERE id = %s", (admin_id,))
            result = cur.fetchone()
            profile_pic_path = result['profile_pic'] if result else None

            # Fetch all users
            cur.execute("SELECT id, username, email, is_admin, status FROM users")
            users = cur.fetchall()
            ...
            # Count pending user registration requests
            cur.execute("SELECT COUNT(*) AS count FROM user_registration_requests WHERE processed = FALSE")
            pending_user_registration_request_count = cur.fetchone()['count']

            # Count pending business registration requests
            cur.execute("SELECT COUNT(*) AS count FROM business_registration_requests WHERE processed = FALSE")
            pending_business_registration_requests_count = cur.fetchone()['count']

            # Count users with pending status
            cur.execute("SELECT COUNT(*) AS count FROM users WHERE status = 'pending'")
            pending_user_count = cur.fetchone()['count']
...
def update_business(business_id):
    ...
    user_id = business['owner_id']  # Assuming owner_id is the second column in the businesses table
    cur.close()
    conn.close()
